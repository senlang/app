...
unsigned char Keyboard(void)
{
	...
	if(KeyScan(GPIOB, KEY0) == KEYDOWN && !(keyBusyFlag & (~(1 << 0))))		//如果按下 且其他按键未按下
	{
	...
	else if(KeyScan(GPIOB, KEY0) == KEYUP && keyBusyFlag & (1 << 0))		//如果释放 且 按键之前是按下过的
	{
	...
	/********************************************下同**********************************************/
	if(KeyScan(GPIOB, KEY1) == KEYDOWN && !(keyBusyFlag & (~(1 << 1))))		//如果按下 且其他按键未按下
	{
	...
	else if(KeyScan(GPIOB, KEY1) == KEYUP && keyBusyFlag & (1 << 1))		//如果释放
	{
	...
	if(KeyScan(GPIOB, KEY2) == KEYDOWN && !(keyBusyFlag & (~(1 << 2))))		//如果按下 且其他按键未按下
	{
	...
	else if(KeyScan(GPIOB, KEY2) == KEYUP && keyBusyFlag & (1 << 2))		//如果释放
	{
	...
	if(KeyScan(GPIOB, KEY3) == KEYDOWN && !(keyBusyFlag & (~(1 << 3))))		//如果按下 且其他按键未按下
	{
	...
	else if(KeyScan(GPIOB, KEY3) == KEYUP && keyBusyFlag & (1 << 3))		//如果释放
	{
